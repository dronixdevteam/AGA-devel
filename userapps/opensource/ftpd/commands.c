...
void do_fwUpdate(void)
{
    ...
    // get interface socket pid and if pid != 0 add the default route before the bind to the default ifc
	...
	for (;;) {
        ...
        if (curPtr == NULL)
        {
            ...
            // the totalImageSize field to determine the size of buffer to
            // allocate.
            if( tagSize + byteRd > TAG_LEN)
                size = TAG_LEN - tagSize;
            else
                size = byteRd;
            memcpy(tag + tagSize, buffer, size);
            tagSize += size;
            byteRd -= size;
            if( tagSize == TAG_LEN )
            {
                ...
                if (verifyTag(pTag, 1) == UPLOAD_OK)
                {
                    ...
                    // add tag len plus some buffer for over flow during the sending...
                    totalAllocatedSize = totalImageSize + TAG_LEN + 8;
                    printf( "Allocating %d byte buffer for broadcom image.\n", totalAllocatedSize );
                    ...
                    else
                    {
                        ...
                        curPtr += TAG_LEN;
                        uploadSize += TAG_LEN;
                        memcpy( curPtr, buffer + size, byteRd);
                        curPtr += byteRd;
                    }
                }
                ...
                {   
                    ...
                    printf("Allocating %d byte buffer for flash image.\n", totalAllocatedSize);
        ...
        uploadSize += byteRd;
//printf("bufsize = %d, uploadSize= %d\n", xfer_bufsize, uploadSize);
        if (uploadSize > totalAllocatedSize)        // try not to over flow the buffer
        {
...
//printf("total size=%d\n", uploadSize);
    ...
    if ((imageType = parseImageData(imagePtr, uploadSize, BUF_ALL_TYPES)) != NO_IMAGE_FORMAT)
    {
       // if it is brcm_image and chip id mismatched.
       if (imageType == BROADCOM_IMAGE_FORMAT && checkChipId(pTag->chipId, pTag->signiture_2) != 0)
       {
           displayMessage(UPLOAD_FAIL_ILLEGAL_IMAGE);
           imageType = NO_IMAGE_FORMAT;
       }
       else
           displayMessage(UPLOAD_OK);  // flash image will be done later
    }
    else 
	    displayMessage(UPLOAD_FAIL_ILLEGAL_IMAGE);
...
void command_quit(char *params)
{
    ...
    if (fNeedReset) 
    {
        fQuitCmd = TRUE;  // brcm set our flash image flag
        if (imageType == NO_IMAGE_FORMAT)
    	    control_printf(SL_SUCCESS, "221 %s", "The Router is rebooting...");
        else
	        control_printf(SL_SUCCESS, "221 %s", "The Router will reboot upon completion (about 2 minutes)...");
    }
    else 
    {
...
// printf("commands[i].name=%s\n", commands[i].name); // brcm
		...
		if (!strncasecmp(str, commands[i].name, strlen(commands[i].name))) {
			...
			if (state >= commands[i].state_needed) {
                ...
                // brcm add code for exit ftp gracefully
