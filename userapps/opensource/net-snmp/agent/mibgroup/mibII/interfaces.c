...
#define BRCM_IFLASTCHANGE_SUPPORT
...
#else
#ifdef BRCM_IFLASTCHANGE_SUPPORT
	/* brcm - if_lastchange now supported */
        long_return = ifnet.if_lastchange;
#else
#if NO_DUMMY_VALUES
        return NULL;
#endif
    ...
    while (fgets(line, sizeof(line), devin)) {
        ...
        *ptr = 0;
        ...
        if (ioctl(fd, SIOCGIFHWADDR, &ifrq) < 0)
            ...
            case ARPHRD_ETHER:
                nnew->if_type = IF_IANAIFTYPE_ETHERNETCSMACD;
                break;
            case ARPHRD_TUNNEL:
            case ARPHRD_TUNNEL6:
#ifdef ARPHRD_IPGRE
...
#endif
            case ARPHRD_SIT:
                nnew->if_type = IF_IANAIFTYPE_TUNNEL;
                break;          /* tunnel */
            case ARPHRD_SLIP:
            case ARPHRD_CSLIP:
            case ARPHRD_SLIP6:
            case ARPHRD_CSLIP6:
                nnew->if_type = IF_IANAIFTYPE_SLIP;
                break;          /* slip */
            case ARPHRD_PPP:
                nnew->if_type = IF_IANAIFTYPE_PPP;
                break;          /* ppp */
            case ARPHRD_LOOPBACK:
                nnew->if_type = IF_IANAIFTYPE_SOFTWARELOOPBACK;
                break;          /* softwareLoopback */
            case ARPHRD_FDDI:
                nnew->if_type = IF_IANAIFTYPE_FDDI;
                break;
            case ARPHRD_ARCNET:
                nnew->if_type = IF_IANAIFTYPE_ARCNETPLUS;
                break;
            case ARPHRD_LOCALTLK:
                nnew->if_type = IF_IANAIFTYPE_LOCALTALK;
                break;
#ifdef ARPHRD_HIPPI
            case ARPHRD_HIPPI:
                nnew->if_type = IF_IANAIFTYPE_HIPPI;
                break;
#endif
#ifdef ARPHRD_ATM
            case ARPHRD_ATM:
                nnew->if_type = IF_IANAIFTYPE_ATM;
                break;
#endif
// brcm
            case ARPHRD_CPCS:
                nnew->if_type = IF_IANAIFTYPE_AAL5;
                break;
            case ARPHRD_DSL:
                nnew->if_type = IF_IANAIFTYPE_ADSL;
                break;
        ...
        strncpy(ifrq.ifr_name, ifname, sizeof(ifrq.ifr_name));
        ifrq.ifr_name[ sizeof(ifrq.ifr_name)-1 ] = 0;
        nnew->if_lastchange = (ioctl(fd, SIOCGIFTRANSSTART, &ifrq) < 0)
            ? 0 : ifrq.ifr_ifru.ifru_ivalue;
#else
        nnew->if_lastchange = 0;
#endif
...
Interface_Index_By_Name(char *Name, int Len)
{
    ...
    return ifIndex;
#else
    /*brcm: if an interface is not found, we should return 0 rather than the
      last index in the interface list */
    while ((Interface_Scan_Next(&ifIndex, ifName, NULL, NULL)) != 0)
    {
	     ...
	     u_char * statP, oid * name, size_t name_len)
{
  ...
  switch (action) {
	...
	return SNMP_ERR_WRONGLENGTH;
	  ...
	  (value != IF_ADMIN_STATUS_DOWN)) {
	snmp_log(LOG_ERR, "not supported admin state %d\n",value);
	return SNMP_ERR_WRONGVALUE;
      }
      break;
    ...
    case COMMIT:               /* Confirm the SET, performing any irreversible actions,
      ...
      value = *((int*)var_val);
      if ((fd = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
	snmp_log(LOG_ERR, "socket open failure in WriteIfEntry\n");
	return SNMP_ERR_COMMITFAILED;
      }
	...
	snmp_log(LOG_ERR, "WriteIfEntry unable to locate device name for ifIndex %d\n",index);
	return SNMP_ERR_COMMITFAILED;
      }
