...
#ifdef BRCM_SNMP_INFO
#include "version.h"
#endif

#include <signal.h>   // need for SIGUSR1
...
system_store(int a, int b, void *c, void *d)
{
    ...
    FILE *fs = fopen("/var/run/httpd_pid", "r");
    ...
    // send SIGUSR1 to httpd for storing SNMP set to PSI
    if ( fs != NULL ) {
       if ( fgets(line, SNMP_MAXBUF_SMALL - 1, fs) != NULL ) {
          pid = atoi(line);
          if ( pid > 0 )
             kill(pid, SIGUSR1);
       }
...
{
#ifdef BRCM_SNMP_INFO
  sprintf(version_descr, "U.S. Robotics Software Version %s", SOFTWARE_VERSION);
#else
#ifdef HAVE_UNAME
...
#endif
#endif
#endif /* BRCM_SNMP_INFO */

#ifdef BRCM_SNMP_INFO
    strncpy(sysName,"U.S. Robotics",sizeof(sysName));
#else
