...
#endif /* #ifdef BRCM_SNMP_MIB_SUPPORT */
...
#define CONTEXTNAMEINDEXLEN 32
    ...
    if (gp == NULL) {
        DEBUGMSG(("mibII/vacm_vars", "group entry NULL\n"));
        return 2;
    }
    ...
    if (ap == NULL) {
        DEBUGMSG(("mibII/vacm_vars", "access entry NULL\n"));
        return 3;
    }
    ...
    if (check_subtree) {
        if (!vp) {
	    DEBUGMSG(("mibII/vacm_vars", "getViewEntry vp is NULL, return subtree unknown"));
            return VACM_SUBTREE_UNKNOWN;
        }
        if (vp->viewType == SNMP_VIEW_EXCLUDED) {
	    DEBUGMSG(("mibII/vacm_vars", "view excluded, return not_in_view"));
            return VACM_NOTINVIEW;
        }
        return VACM_SUCCESS;
    }
    if (vp == NULL) {
        DEBUGMSG(("mibII/vacm_vars", "vp is NULL, return 4\n"));
        return 4;
    }
    ...
    if (vp->viewType == SNMP_VIEW_EXCLUDED) {
	    ...
	    DEBUGMSG(("mibII/vacm_vars", "viewtype excluded, bad in bad snmp community, returns 5"));
