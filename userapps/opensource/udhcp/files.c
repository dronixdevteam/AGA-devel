...
int read_config_static_leases(char *file)
{
	FILE *in;
	char buffer[READ_CONFIG_BUF_SIZE], *token, *line;
	int i;

	for (i = 0; strlen(keywords_static_leases[i].keyword); i++)
		if (strlen(keywords_static_leases[i].def))
			keywords_static_leases[i].handler(keywords_static_leases[i].def, keywords_static_leases[i].var);

	if (!(in = fopen(file, "r"))) {
		LOG(LOG_ERR, "static leases file not found: %s", file);
		return 0;
	}
		...
		*line = '\0';
		...
		for (i = 0; strlen(keywords_static_leases[i].keyword); i++)
			if (!strcasecmp(token, keywords_static_leases[i].keyword))
				keywords_static_leases[i].handler(line, keywords_static_leases[i].var);
	}
	fclose(in);
	return 1;
}
