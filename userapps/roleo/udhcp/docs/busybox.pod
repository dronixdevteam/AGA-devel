...
dhcpc [B<-fbnqv>] [B<-c> CLIENTID] [B<-H> HOSTNAME] [B<-i> INTERFACE]
...
tftp [OPTION]... tftp_server_ip

Update firmware image and configuration data from OR backup configuration
data to a tftp server.

Options:
B<-g>	Get file. (Update image/configuration data)
B<-p>	Put file. (backup configuration data)
B<-f>	remote file name.
B<-t>	i for image and c for configuration data.
