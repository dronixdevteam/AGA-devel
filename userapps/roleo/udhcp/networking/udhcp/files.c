...
static const struct config_keyword keywords_static_leases[] = {
	/* keyword	handler   variable address		default */
	{"static_lease",read_staticlease, &(server_config.static_leases),	""},
	/*ADDME: static lease */
	{"",		NULL, 	  NULL,				""}
...
int read_config_static_leases(const char *file)
{
	FILE *in;
	char buffer[READ_CONFIG_BUF_SIZE], *token, *line;
#ifdef UDHCP_DEBUG
	...
	for (i = 0; keywords_static_leases[i].keyword[0]; i++)
		if (keywords_static_leases[i].def[0])
			keywords_static_leases[i].handler(keywords_static_leases[i].def, keywords_static_leases[i].var);

	if (!(in = fopen(file, "r"))) {
		LOG(LOG_INFO, "static leases file not found: %s", file);
		return 0;
	}
	...
	while (fgets(buffer, READ_CONFIG_BUF_SIZE, in)) {
		...
		for (i = 0; keywords_static_leases[i].keyword[0]; i++)
			if (!strcasecmp(token, keywords_static_leases[i].keyword))
				if (!keywords_static_leases[i].handler(line, keywords_static_leases[i].var)) {
					LOG(LOG_ERR, "Failure parsing line %d of %s", lm, file);
					DEBUG(LOG_ERR, "unable to parse '%s'", orig);
					/* reset back to the default value */
					keywords_static_leases[i].handler(keywords_static_leases[i].def, keywords_static_leases[i].var);
...
void read_leases(const char *file)
{
	...
	struct dhcpOfferedAddr *added_lease;
	...
	while (i < server_config.max_leases && (fread(&lease, sizeof lease, 1, fp) == 1)) {
		...
		if (lease.yiaddr >= server_config.start && lease.yiaddr <= server_config.end) {
			...
			added_lease = add_lease(lease.chaddr, lease.yiaddr, lease.expires);
			if (!added_lease) {
				LOG(LOG_WARNING, "Too many leases while loading %s\n", file);
				break;
			}
			memcpy(added_lease->hostname, lease.hostname, sizeof(lease.hostname));
			i++;
		}
	}
