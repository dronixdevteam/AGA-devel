...
/*    if (verifyAccessControl() == 1) {
	...
	// SAMBA
		...
		SetSambaRsiConf(ena,entries[3].val, entries[4].val, entries[5].val, rs);
		sprintf(strFileToLoad, EVAL_HTML_FILE);
	...
	//PARTITIONCHECK
	    ...
	    } else if (strncmp("start", entries[1].val, 5) == 0) {
	        ExecFsck(entries[2].val);
	        sprintf(strFileToLoad, EVAL_HTML_FILE);
	        sprintf(strEval, "page=partitioncheck&action=view&device=%s", entries[2].val);
	    }
	//PARTITIONCREATE
	} else if (strncmp("partitioncreate", entries[0].val, 15) == 0) {
	    if (strncmp("view", entries[1].val, 4) == 0) {
	        sprintf(strArg, "%s", entries[2].val);
	    } else if (strncmp("startfdisk", entries[1].val, 10) == 0) {
	        ExecFdisk(entries[2].val);
	        sprintf(strFileToLoad, EVAL_HTML_FILE);
	        sprintf(strEval, "page=partitioncreate&action=view&device=%s", entries[2].val);
	    } else if (strncmp("startformat", entries[1].val, 11) == 0) {
	        ExecFormat(entries[2].val);
	        sprintf(strFileToLoad, EVAL_HTML_FILE);
	        sprintf(strEval, "page=partitioncreate&action=view&device=%s", entries[2].val);
	    }
	//WAKE ON LAN
	...
	// WEBCAM SERVER
	} else if (strncmp("webcam", entries[0].val, 13) == 0) {
	    if (numEntries != 12) {
		sprintf(strFileToLoad, STD_HTML_FILE);
		sprintf(strStdTitle, "webcam configuration");
		sprintf(strStdText, "Generic error");    
	    } else if (strncmp("modify", entries[1].val, 6) == 0) {
		int ena = 0;
		int wan = 0;
		...
		width = strtol(entries[6].val, NULL, 10);
		if ((errno == EINVAL) || (errno == ERANGE)) {
		    sprintf(strFileToLoad, STD_HTML_FILE);
		    sprintf(strStdTitle, "webcam configuration");
		    sprintf(strStdText, "Generic error");    
		} else {
		    height = strtol(entries[7].val, NULL, 10);
		    if ((errno == EINVAL) || (errno == ERANGE)) {
			sprintf(strFileToLoad, STD_HTML_FILE);
			sprintf(strStdTitle, "webcam configuration");
			sprintf(strStdText, "Generic error");    
		    } else {
			fps = strtol(entries[8].val, NULL, 10);
			if ((errno == EINVAL) || (errno == ERANGE)) {
			    sprintf(strFileToLoad, STD_HTML_FILE);
			    sprintf(strStdTitle, "webcam configuration");
			    sprintf(strStdText, "Generic error");    
			} else {
			    quality = strtol(entries[9].val, NULL, 10);
			    if ((errno == EINVAL) || (errno == ERANGE)) {
				sprintf(strFileToLoad, STD_HTML_FILE);
				sprintf(strStdTitle, "webcam configuration");
				sprintf(strStdText, "Generic error");    
			    } else {
				tcpport = strtol(entries[10].val, NULL, 10);
				if ((errno == EINVAL) || (errno == ERANGE)) {
				    sprintf(strFileToLoad, STD_HTML_FILE);
				    sprintf(strStdTitle, "webcam configuration");
				    sprintf(strStdText, "Generic error");
				} else {
				    oldtcpport = strtol(entries[11].val, NULL, 10);
				    if ((errno == EINVAL) || (errno == ERANGE)) {
					sprintf(strFileToLoad, STD_HTML_FILE);
					sprintf(strStdTitle, "webcam configuration");
					sprintf(strStdText, "Generic error");    
				    } else {
					SetWebcamRsiConf(ena, entries[3].val, wan, entries[5].val, width, height, fps, quality, tcpport, oldtcpport);
					sprintf(strFileToLoad, EVAL_HTML_FILE);
					sprintf(strEval, "page=webcam");
				    }
				}
			    }
			}
	...
	if (strcasecmp("GetStatus()", tag) == 0) {
	    ...
	    GetFsckOut(strArg);
	} else if (strcasecmp("GetDisks()", tag) == 0) {
	    GetDisks();
	} else if (strcasecmp("GetFdiskVar()", tag) == 0) {
	    GetFdiskVar();
	} else if (strcasecmp("GetFdiskRefresh()", tag) == 0) {
	    GetFdiskRefresh(2);
	} else if (strcasecmp("GetFdiskStatus()", tag) == 0) {
	    GetFdiskStatus(strArg);
	} else if (strcasecmp("GetFdiskOut()", tag) == 0) {
	    GetFdiskOut(strArg);
	...
	} else if (strcasecmp("GetWebcamConf()", tag) == 0) {
	    GetWebcamConf();
	} else if (strcasecmp("GetWebcamViewConf()", tag) == 0) {
	    GetWebcamViewConf();
...
int verifyAccessControl()
{
    ...
    fp = popen("iptables -L INPUT -n 2>/dev/null", "r");   
    if (fp != NULL) {
